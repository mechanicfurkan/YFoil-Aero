...
The physics modules constituting the core of the application are: the International Standard Atmosphere (ISA) model, the viscous-inviscid interaction (VIE) framework consisting of the Hess-Smith source-vortex panel method and the Head integral boundary layer solver, a robust 3-Tier Aerodynamic Solver (Vortex Lattice Method, Non-Linear Lifting-Line Theory, and Linear Lifting-Line Theory), the experimental NACA polar database by Abbott and von Doenhoff (1959), the Karman-Tsien and Laitone blended compressibility corrections, the wave drag model, the Michel transition criterion, and the Schlichting hybrid boundary layer friction formulation.  
...
| 2D Panel Solver | Hess-Smith Source-Vortex (80 panels) |
| 3D Solver Architecture | 3-Tier Fallback (VLM → NLLT → pLLT) |
| VLM Discretization | Thin-wing Horseshoe Vortices (20 spanwise panels) |
| NLLT Discretization | 30 Fourier Modes |
| Boundary Layer | Head Integral Method (VIE) |
| Polar Database | Abbott & von Doenhoff (1959) — 18 NACA Airfoils |
| Compressibility | Blended Karman-Tsien & Laitone Corrections |
| Dynamic Stability | Analytical $C_{L_q}$ and $C_{m_q}$ tracking |
...
* **Physical Accuracy:** The models used should not consist solely of empirical regressions; instead, physical-based semi-analytical methods (VLM, panel solver, integral boundary layer) are combined with experimental data (Abbott & von Doenhoff PDB).  
...
* Static margin, neutral point, and dynamic stability derivatives (CLq, Cmq)  
* Root bending moment  
* Wave drag & Blended Compressibility (M \> Mcr)  
* Ground effect  
...
## **6\. Core Aerodynamic Engine: 3-Tier Fallback Solver**

To calculate lift and induced drag distribution over a 3D wing, YFoil Aero employs a highly robust 3-tier fallback architecture. This ensures that the simulator prioritizes the most advanced 3D aerodynamic method while maintaining absolute stability in extreme flight regimes by falling back to simpler models if convergence fails.

### **6.1 Tier 1: Vortex Lattice Method (VLM)**
VLM is the primary engine of the simulation, offering true 3D modeling of the wing geometry. 
* **Theoretical Basis:** It is based on thin-airfoil theory using horseshoe vortices placed along the spanwise panels.
* **Biot-Savart Law:** The induced velocity at each control point is evaluated using the Biot-Savart law. The horizontal bound vortex segments generate the primary lifting force, while the trailing vortices extending to infinity model the downwash effect.
* **Solution Matrix:** A Boundary Condition influence matrix is formed ensuring zero flow normal to the panels (V·n = 0). This matrix is solved via Gaussian Elimination to determine the circulation strength ($\Gamma$) of each panel.
* **Singularity-Free Induced Drag (Trefftz Plane):** To prevent enormous error spikes caused by evaluating downwash directly on the bound vortex control points, YFoil Aero implements the Trefftz Plane approximation. Induced drag (CDi) is calculated strictly by isolating the downwash effect from the trailing vortices, yielding industrial-grade drag estimations.

### **6.2 Tier 2: Non-Linear Lifting-Line Theory (NLLT)**
If the VLM solver encounters non-physical geometries or extreme angles of attack, the engine falls back to NLLT.
* **Fourier Series Formulation:** The circulation distribution $\Gamma(y)$ along the wing is represented as a Fourier sine series with 30 modes.
* **Non-Linear Iteration:** The local effective angle of attack is computed, and the 2D airfoil database (PDB) is queried to inject non-linear phenomena such as stall onset and viscous effects back into the 3D calculation. The loop runs until a tolerance of $\Delta C_L < 10^{-4}$ is reached.

### **6.3 Tier 3: Linear Lifting-Line Theory (pLLT)**
If the NLLT iteration reaches its maximum limit (18 iterations) without converging, the engine relies on Prandtl's classic Linear Lifting Line Theory. This single-matrix solution is unconditionally stable and acts as the ultimate fail-safe.

## **7\. Drag Models & Compressibility**
...
### **7.1 Compressibility Corrections**
To account for Mach effects before sonic speeds are reached, YFoil Aero utilizes a blended compressibility correction. For Mach numbers approaching 0.55 and above, the solver seamlessly transitions between the **Karman-Tsien** rule and the **Laitone** correction rule. This blended logic ensures that local pressure coefficients ($C_p$) and lift slopes ($C_{L\alpha}$) scale accurately without premature divergence at high subsonic Mach numbers.
...
## **9\. Longitudinal Balance and Dynamic Stability Analysis**

YFoil Aero performs instantaneous computations for both static and dynamic flight mechanics parameters.

### **9.1 Static Margin & Neutral Point**
Longitudinal static balance analysis is performed by taking the numerical derivative from the polar sweep data. The Neutral Point ($x_{np}$) is calculated, and the Static Margin is determined relative to the user-defined Center of Gravity ($x_{cg}$). The root bending moment is also calculated via trapezoidal integration using the spanwise load distribution data.

### **9.2 Dynamic Stability Derivatives**
To support flight simulation and control systems, the following dynamic derivatives are solved analytically:
* **Pitch-Rate Lift Derivative ($C_{L_q}$):** Represents the additional lift generated due to the aircraft pitching upwards. It is calculated by modeling the induced camber effect across the wing.
* **Pitch-Damping Derivative ($C_{m_q}$):** This is one of the most vital parameters for longitudinal dynamic stability, representing how quickly pitch oscillations dampen out. It is formulated utilizing the wing area, aspect ratio, and the offset between the aerodynamic center and the center of gravity.
